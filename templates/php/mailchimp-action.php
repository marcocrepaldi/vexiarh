...
if ( isset( $_POST['url'] ) ) {
	...
	if ( ! empty( $email ) && ! filter_var( $email, FILTER_VALIDATE_EMAIL ) === false ) {
		...
		// MailChimp API credentials (configure via environment variables)
		$list_id     = getenv('MAILCHIMP_LIST_ID') ?: '';
		$api_key     = getenv('MAILCHIMP_API_KEY') ?: '';
		$doubleOptIn = true;

		// Block request if credentials are missing
		if (empty($list_id) || empty($api_key)) {
			echo json_encode('<p class="mc-error-notice">Mailchimp credentials not configured.</p>');
			die();
		}
